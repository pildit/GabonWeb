  ...
  <div class="card" ref="development-unit-form">
    ...
    <div class="card-body px-lg-5 pt-0" v-permission="'development-unit.add'">
      ...
      >
          ...
          <div class="col">
            <div class="md-form">
              <input
                ...
                v-validate="'required'"
              />
              <label for="Number" :class="{ active: form.Number }">{{
                translate("number_development_unit_form")
              }}</label>
              <div v-show="errors.has('Number')" class="invalid-feedback">
                {{ errors.first("Number") }}
              </div>
            </div>
          </div>
          <div class="col">
            <div class="md-form">
              <input
                ...
                v-validate="'required'"
              />
              <label for="Name" :class="{ active: form.Name }">{{
        ...
        <div class="form-row">
          <div class="col">
            <div class="md-form">
              ...
              />
              <label for="Geometry" :class="{ active: form.Geometry }">{{
            ...
            <div class="md-form">
              ...
              >
                <template slot="singleLabel" slot-scope="{ option }"
                  >{{ option.Name }}({{ option.Id }})</template
                >
                <template slot="option" slot-scope="{ option }"
                  >{{ option.Name }}({{ option.Id }})</template
                >
              </multiselect>
              <div v-show="errors.has('concession')" class="invalid-feedback">
        ...
        <div class="form-row">
          <div class="col">
            <div class="md-form mt-3">
              ...
              <div v-show="errors.has('dates')" class="invalid-feedback">
                {{ errors.first("dates") }}
              </div>
            </div>
          </div>
        ...
        ></plan-form-partial>
        <div class="form-row">
          <a class="btn btn-info" @click="addPlan()">{{
        ...
        <div class="form-row float-right">
          ...
          <a class="btn btn-warning z-depth-0 my-4" :href="indexRoute()">{{
  ...
  watch: {
    developmentUnitProp(value) {
      if (value) {
        ...
        const fakeGeometry =
          "POLYGON((1.5598658653430082 12.148603500672486,1.4061088354351594 10.63374624330127,0.05493163220967156 10.567882884285124,0.2856433479945185 12.818214317336558))";
        this.form.Geometry = fakeGeometry; // TODO: Remove this shit
        // EventBus.$emit(this.endpointName, (this.form.Geometry))
        if (
          value.geometry_as_text !== this.form.Geometry &&
          this.endpointEdit
        ) {
